...
function unblock(
  ...
  if (confirm("确认要解除对 " + name + " 的屏蔽？")) {
    $.get("https://www.v2ex.com/unblock/" + id + "?t=" + created, function(
      data,
      status
    ) {
      if (status == "success") {
...
$(function() {
  if (block_list.length == 1 && block_list[0] == "") {
    $("#blockList").append(
      "<table><td>当前账号无任何屏蔽用户，请检查是否登录了正确的账号。</td></table>"
    );
    return;
  }

  for (let user_id of block_list) {
    $.get("https://www.v2ex.com/api/members/show.json?id=" + user_id, function(
      data,
      status
    ) {
      if (status == "success") {
          ...
          $("#blockList").append(
            $("<table>").append(
              $("<td>").text(
                "ID为 " +
                  user_id +
                  " 的用户似乎已经找不到，有可能是本次查询出错或查询次数超出限制或该用户已经被删除。"
              )
            )
          );
        }
