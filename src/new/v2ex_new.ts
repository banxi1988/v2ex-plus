...
//插入图片
function input_img(input_img_base64: string, this_img_id: number) {
  $("#imgManage").append(
    $("<div>", { class: "imgId" + this_img_id })
      .append(
        $("<div>").append(
          $("<img>", {
            class: "imgPreview",
            src: input_img_base64,
            alt: "上传图片"
          })
        )
      )
      .append($("<input>", { style: "text", onmouseover: "this.select();" }))
      .append(
        $("<img>", {
          class: "deleteBTN",
          src: deleteBTN_img,
          onclick: '$(".imgId' + this_img_id + '").remove();'
        })
      )
  );
...
$("body").append(
  "<div id='imgFun'><div id='imgManage' class='box'>粘贴截图或<span id='imgUploadBtn' style='cursor: pointer;'>上传图片</span><input type='file' style='display:none;' id='imgUpload' accept='image/*' /></div><div id='imgFunBtn'>&emsp;<</div></div>"
);
...
_imgUpload.change(function(e: any) {
